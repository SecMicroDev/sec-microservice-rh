...
def test_create_enterprise(test_client_authenticated_default: TestClient):
    ...
    enterprise_data = BaseEnterprise(name="Test Enterprise", accountable_email="enterprise_test@test.mail.com", activity_type="Test Activity")
    user_data = FirstUserCreate(username="testuser", password="testpassword", email="enterprise_test@test.mail.com")
    response = client.post("/enterprise/signup", json={"enterprise": enterprise_data.model_dump(), "user": user_data.model_dump()})
...
def test_update_enterprise(test_client_authenticated_default: TestClient):
    ...
    enterprise_data = EnterpriseUpdate(name="Updated Test Enterprise", description="Updated Test Description")
